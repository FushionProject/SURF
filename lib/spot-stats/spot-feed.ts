...
export function buildSpotFeed(input: {
  ...
  for (const game of result.games) for (const side of ["away", "home"] as const) {
    ...
    const projection = qb ? `${qb.name} is the projected QB in the saved schedule for the ${name}; this trend applies only if he starts. These are team results, not individual passing statistics.` : "";
    ...
    if (qb) addRecency(game, team, "qb", { streak: "QB · Current streak", recent: "QB · Recent form · Last 10" }, qb.name, "his", "starts", {
      ...
      recent: `${qb.name} is the projected QB in the saved schedule for the ${name}; this trend applies only if he starts. These are team results in his ${RECENT_FORM_GAMES} most recent regular-season starts, listed most recent first, with any team and across seasons, not individual passing statistics. ${recentExplanation}`,
    }, starts, false);
