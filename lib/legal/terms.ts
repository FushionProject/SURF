import { LEGAL_CONTACT } from "./shared.ts";

export const TERMS_BODY = `## 1. Acceptance

These terms cover your use of Surf, the web app at surfodds.com. By using Surf, you agree to them. If you do not agree, please do not use Surf.

Surf is an independent project. No company has been formed for it yet. When one is, this page will name it.

## 2. What Surf is

Surf displays US sports betting market data and Surf's own analysis derived from it. Coverage is NFL-first, and also includes MLB and college football.

Surf is an information and analysis product. It is not a picks service, not a tipster service, and it does not tell you what to bet. Every decision you make is yours.

These features are free: **Game briefs** (sportsbook odds compared across books, market midpoint, best available number, recorded line movement, injury context, prediction-market consensus), **Spot Stats** for one featured matchup each week (historical NFL situational records, win-loss and against the spread), **Signals** for that same featured matchup, **Watchlist**, and **How to use Surf**. You can use the free board without an account. While billing is turned off, the full Signals and Spot Stats slates are open to everyone.

One paid plan is planned but is not active: **Surf Pro** at $9.99 per month, which unlocks Signals and Spot Stats for every matchup on the slate. Billing is turned off. Nobody can subscribe today and Surf is not accepting payments. See section 10.

## 3. What Surf does not do

Read this section carefully. It is the most important part of these terms.

- Surf does not accept or place wagers.
- Surf does not hold customer funds.
- Surf does not ask for or store your sportsbook credentials.
- Surf is not a sportsbook and is not a broker.
- Surf does not guarantee profit, accuracy, edge, or any outcome.

Historical records in Spot Stats are descriptive. They report what happened in past situations. They do not predict future results, and a pattern that held before may not hold again.

Odds, injury information and prediction-market data can be delayed, incomplete or wrong. Confirm a number at the sportsbook before you act on it.

## 4. Where the data comes from

Surf displays data from third parties and does not own that data.

- **The Odds API** — sportsbook odds.
- **API-Sports** — NFL injuries and college football context.
- **ESPN** — AP Top 25 rankings and team logos.
- **nflverse** — historical NFL schedules and results, used under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) with attribution.
- **Polymarket** — public prediction-market data.

Any source may change its terms, change its data, or stop being available. Parts of Surf that depend on a source may change or disappear.

## 5. Links to sportsbooks

Surf has no affiliate compensation arrangement with any sportsbook. We are not paid when you click out.

Outbound links use event links supplied by our data provider. Links matching betslip, stake, deposit or redirect patterns are rejected and not shown.

What happens after you leave Surf is between you and that sportsbook.

## 6. Eligibility and where Surf is offered

You must be at least 18 to use Surf.

Surf is intended for users in the **United States**. It is not directed at people in other territories. Betting laws differ by state. You are responsible for knowing whether sports betting is legal where you are, and for following the law where you are. In most US states the legal betting age is 21, which is higher than the age required to use Surf.

## 7. Accounts and security

An account is optional. To create one you give an email address and a password, and confirm the email address.

One account per person. Keep your password to yourself. You are responsible for what happens under your account. If you think someone else has access, change your password and tell us.

We may refuse, suspend or remove an account that breaks these terms.

## 8. Acceptable use

You may read Surf, use its features, and act on what you learn however you like. You may not:

- Scrape Surf or extract data from it in bulk.
- Resell or redistribute the underlying datasets shown on Surf.
- Use bots or crawlers to harvest data from Surf.
- Reverse engineer Surf or try to reach its source code or internal logic.
- Reproduce Surf's data or analysis as a competing feed, product or service.
- Break into accounts, probe for weaknesses, or interfere with the site.
- Use Surf in a way that breaks the law, or that breaks the terms of the sources in section 4.

## 9. Intellectual property

Surf's interface, design, code, written explanations and its own derived analysis belong to Surf.

The third-party data in section 4 belongs to its owners, as do their names, logos and marks, including team logos and league marks. Nothing here transfers rights in that material to you, and nothing on Surf implies any of those parties endorse Surf or are connected to it.

nflverse data is used under CC BY 4.0, with attribution shown on the site.

## 10. Paid subscriptions

Billing is disabled. This section describes how paid plans will work and takes effect only if billing is switched on. Until then nothing here obliges you to pay anything, and Surf cannot charge you.

When billing is enabled:

- **Plan.** Surf Pro at $9.99 per month, covering Signals and Spot Stats for every matchup. One featured matchup a week stays free without a plan. The price is in US dollars and excludes any applicable tax.
- **Payments.** Handled by Stripe. Surf will not store your full card details.
- **Renewal, cancellation and refunds.** A subscription renews automatically each month until you cancel. Section 11 sets out how to cancel and what is refundable.
- **Price changes.** A new price applies to renewals at least 30 days after we tell you.

We will not charge anyone without asking them to sign up first.

## 11. Cancellation and Refunds

This section covers how a paid Surf subscription ends and what is refundable. It applies to every paid plan.

- **Monthly and automatic.** Surf subscriptions are billed monthly and renew automatically each month until you cancel.
- **Cancel at any time.** You can cancel yourself from your [account page](/account). You do not need to email us or ask permission, and there is no cancellation fee.
- **When cancelling takes effect.** Cancellation takes effect at the end of the billing period you have already paid for. Access continues until that date, and you are not charged again after it.
- **No partial refunds.** Surf does not give prorated or partial-month refunds for time already elapsed. If you cancel halfway through a month, you keep access for the rest of that month and that month's payment is not refunded.
- **Where the law gives you more.** Some places give consumers refund rights that these terms cannot remove. Where that is true, those rights apply.
- **Billing problems.** If you think you were charged in error, charged after cancelling, or charged twice, email [${LEGAL_CONTACT}](mailto:${LEGAL_CONTACT}). Tell us the email address on the account and the date of the charge, and we will look into it and correct a genuine billing error.

Payments and subscriptions are handled by Stripe. Cancelling from your account page cancels the Stripe subscription behind it, and your card is not charged again once the current period ends.

## 12. No warranty

Surf is provided as it is, with whatever faults it has. We do not promise it will be accurate, complete, current, uninterrupted or available at all.

We do not promise anything you learn on Surf will make you money, that a price shown is still available, or that an injury note is current.

Sports data is messy and sportsbooks change numbers constantly. Treat everything on Surf as a starting point.

## 13. Limitation of liability

To the extent the law allows, Surf is not liable for money you lose betting, for bets you place or fail to place, for missed opportunities, for lost profits, or for indirect or consequential loss connected to your use of Surf.

If Surf is found liable to you, total liability is limited to what you have paid Surf in the twelve months before the claim. Because billing is disabled, that amount is currently zero for everyone.

Some places do not allow parts of this section. Where that is true those parts do not apply to you and the rest still does.

## 14. Responsible gambling

Betting can cause real harm. It is not a way to make a living and not a way to fix a financial problem.

If gambling is causing trouble for you or someone close to you, help exists. In the United States you can call or text the National Problem Gambling Helpline at **1-800-MY-RESET** (1-800-697-3738), or visit [1800myreset.org](https://www.1800myreset.org). Most sportsbooks also offer deposit limits, timeouts and self-exclusion.

Surf shows market data. It does not know your finances and cannot tell you what is a sensible amount to risk.

## 15. Changes

Surf is in active development. Features and data sources may be added, changed or removed at any time.

We may change these terms. When we do, the date at the top changes, and material changes will be described on this page. If you keep using Surf after a change takes effect, that counts as accepting it.

## 16. Termination

You can stop using Surf at any time, and can ask us to delete your account.

We can suspend or close an account, or block access, if you break these terms or to protect the site or other users. We can also stop offering Surf entirely.

Sections 9, 12, 13, 14 and 17 survive.

## 17. Governing law

These terms are governed by the laws of the State of Illinois, without regard to its conflict-of-laws rules.

## 18. Contact

Questions about these terms, account problems, or deletion requests: [${LEGAL_CONTACT}](mailto:${LEGAL_CONTACT}).

---

*These terms have not been reviewed by a lawyer. They should be before Surf launches anything paid.*`;
