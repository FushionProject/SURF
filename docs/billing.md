...
Stripe Checkout and Customer Portal are wired to confirmed Supabase accounts. Billing is **disabled by default**. The proposed catalog is Free, Signals ($9.99 USD/month), and Signals + Spot Stats ($19.99 USD/month). No external products, prices, live charges, or remote account changes were created. Spot Stats purchase and access additionally require `SURF_SPOT_STATS_RELEASE_READY=true`; this remains false until the feature is ready for release.
...
- `AccountBilling` displays the selected subscription tier separately from available plans, recurring amounts, test/live mode, subscription status, and hosted billing buttons. The proposed paid catalog is offered only after Stripe validates both prices against the exact server catalog.
...
2. Configure separate active Surf Products and recurring licensed per-unit monthly USD Prices: `STRIPE_SIGNALS_PRICE_ID` must be exactly 999 cents and `STRIPE_SIGNALS_SPOT_STATS_PRICE_ID` exactly 1999 cents. Both prices and products must be distinct and match test/live mode. The old single `STRIPE_PRICE_ID` is not accepted. Browser requests accept only `{ "planId": "signals" }` or `{ "planId": "signals_spot_stats" }`; no arbitrary prices, customer IDs, quantities, or URLs. Switching tiers with an open checkout fails closed. Concurrent tier requests share the durable idempotency key so they cannot create separate checkouts. Existing subscribers use the portal; configure its plan-switching catalog to exclude unreleased Spot Stats.
