...
Stripe Checkout and Customer Portal are wired to confirmed Supabase accounts. Billing is **disabled by default**. The catalog is Free and **Surf Pro** ($9.99 USD/month), which unlocks Signals and Spot Stats for every matchup. No external products, prices, live charges, or remote account changes were created.

Free viewers (signed out, or signed in without Pro) are not walled off: `viewerHasPro()` in `lib/billing/viewer-access.ts` decides, failing closed, whether the viewer sees the whole slate, and `/stats` and `/api/surf-feed` then show one featured matchup per week in full (`pickFeaturedGame` in `lib/spot-stats/featured.ts`, a pure function of the slate so both surfaces agree) with the rest of the slate locked behind a count and an upgrade link. Whether Spot Stats is visible at all is governed separately by `SURF_SPOT_STATS_PUBLISHED`. `paidFeatureDenial` remains for callers that need a hard denial response.
...
- `AccountBilling` displays the current plan, recurring amount, test/live mode, subscription status, and hosted billing buttons. The Surf Pro checkout button is offered only after Stripe validates the configured price against the exact server catalog.
...
Payment confirmation is never inferred from the success URL. `service.entitlements(userId)` verifies the saved customer association and current Stripe state. Exactly one active, single-item, quantity-one subscription with the configured Surf Pro price and an unexpired period grants access; it sets both the `signals` and `spotStats` entitlements. Trials, overdue/unpaid, paused, expired, unknown-price, duplicate, or malformed subscriptions grant no paid access. The journal is not a substitute for this authoritative check. Provider failures must fail closed at paid-feature boundaries.
...
2. Configure one active Surf Pro Product with a recurring licensed per-unit monthly USD Price of exactly 999 cents, and set `STRIPE_PRO_PRICE_ID` to it. The price and product must match test/live mode. The older `STRIPE_PRICE_ID`, `STRIPE_SIGNALS_PRICE_ID`, and `STRIPE_SIGNALS_SPOT_STATS_PRICE_ID` are not read; `SURF_SPOT_STATS_RELEASE_READY` is no longer read either. Browser requests accept only `{ "planId": "pro" }`; no arbitrary prices, customer IDs, quantities, or URLs. A pending checkout with a different price fails closed. Repeat requests share the durable idempotency key so they cannot create separate checkouts. Existing subscribers use the portal.
