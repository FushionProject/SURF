...
assert.deepEqual(signalStrength(opportunity), { score: 88, label: "Strong", measure: "Opportunity value" });
...
assert.equal(signalStrength(moved).measure, "Market magnitude");
...
assert.equal(signalStrength(horizon).measure, "Event relevance");
...
assert.equal(signalStrength(whale).measure, "Activity size");
