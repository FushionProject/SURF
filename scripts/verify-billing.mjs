...
const { createBillingService } = await import("../lib/billing/service.ts");
const fixtureKey = ["rk", "test", "localfixture"].join("_");
const env = {
  SURF_BILLING_ENABLED: "true", STRIPE_RESTRICTED_KEY: fixtureKey,
  STRIPE_WEBHOOK_SECRET: "whsec_localfixture", STRIPE_PRICE_ID: "price_fixture", NEXT_PUBLIC_SITE_URL: "http://localhost:3000", NODE_ENV: "development",
};
const config = readBillingConfig(env);
assert.ok(config);
for (const field of ["SURF_BILLING_ENABLED", "STRIPE_RESTRICTED_KEY", "STRIPE_PRICE_ID", "STRIPE_WEBHOOK_SECRET", "NEXT_PUBLIC_SITE_URL"]) {
...
const price = { id: "price_fixture", active: true, livemode: false, type: "recurring", recurring: { interval: "month", interval_count: 1, usage_type: "licensed" }, billing_scheme: "per_unit", unit_amount: 1000, currency: "usd", product: { id: "prod_fixture", name: "Fixture plan", active: true } };
const subscription = { id: "sub_fixture", customer: customer.stripe_customer_id, livemode: false, status: "active", cancel_at: null, cancel_at_period_end: false, items: { data: [{ price: { id: price.id }, current_period_end: 1900000000 }] } };
...
function fixture() {
  ...
  let session = { id: "cs_test_fixture", customer: customer.stripe_customer_id, livemode: false, status: "open", url: "https://checkout.stripe.com/c/pay/test", line_items: { data: [{ price: { id: price.id } }] } };
  const store = {
...
assert.equal(status.plan.name, "Fixture plan", "name comes from the configured Stripe product, not an invented tier");
...
await happy.service.checkout(userId);
...
for (const bad of ["active", "trialing", "past_due", "unpaid", "paused", "incomplete"]) {
  ...
  await assert.rejects(test.service.checkout(userId), /already have a subscription/);
  assert.equal(test.calls.some(([call]) => call === "createCheckout"), false);
}
for (const patch of [{ active: false }, { livemode: true }, { unit_amount: 0 }, { type: "one_time" }, { recurring: { ...price.recurring, usage_type: "metered" } }, { product: { deleted: true } }]) {
  const test = fixture(); test.stripe.prices.retrieve = async () => ({ ...price, ...patch });
  await assert.rejects(test.service.checkout(userId));
...
assert.equal(archivedStatus.plan, undefined);
...
await assert.rejects(missingDb.service.checkout(userId), /db unavailable/);
...
await assert.rejects(truncated.service.checkout(userId), /review your billing/);
const pending = fixture(); await pending.service.checkout(userId); pending.session({ status: "complete", subscription: subscription.id });
await assert.rejects(pending.service.checkout(userId), /processing/);
pending.current({ ...subscription, status: "canceled" });
await pending.service.checkout(userId);
assert.equal(pending.calls.filter(([call]) => call === "createCheckout").length, 2, "canceled customer can resubscribe with a new nonce");
const stalePrice = fixture(); await stalePrice.service.checkout(userId); stalePrice.session({ line_items: { data: [{ price: { id: "price_old" } }] } });
await assert.rejects(stalePrice.service.checkout(userId), /different pricing/);
