...
try {
  ...
  captured.set("surf-feed:americanfootball_nfl", { sportKey: "americanfootball_nfl", dataSource: "demo", dataNotice: "Test-only browser fixtures", signals: [middle, whale, { ...whale, id: "ui-expired", commenceTime: new Date(now - 1).toISOString() }] });
  await page.goto(`${preview}/feed?sport=americanfootball_nfl`);
  await page.locator(".bn-current-signals").waitFor();
  assert.equal(await page.locator(".bn-signal-card").count(), 2);
  const first = page.locator(".bn-signal-card").first();
  await first.getByText("$12,000", { exact: true }).waitFor();
