...
const features = [
  ...
  { number: "03", title: "Put history in perspective.", description: "Spot Stats is our upcoming historical research experience. Explore how teams performed in similar situations, with sample sizes and limitations in view.", tag: "SPOT STATS · COMING SOON", href: "#pricing", label: "See the planned bundle" },
];
    ...
    <div className={styles.page}>
      ...
      <main id="main">
        ...
        <section id="pricing" className={styles.section}>
          <div className={styles.sectionHeading}><p className={styles.eyebrow}>CHOOSE YOUR DEPTH</p><h2>Start with the market.<br />Go deeper when you’re ready.</h2><p>Explore the free board today.<br />Paid subscriptions and Spot Stats are coming soon.</p></div>
          <div className={styles.plans}>
            {[{ plan: BILLING_PLANS.free, caption: "Get your bearings.", items: ["Compare available sportsbook quotes", "Explore upcoming matchups", "Learn with Surf’s field guide"], action: "Explore free", href: "/games" }, { plan: BILLING_PLANS.signals, caption: "Read between the lines.", items: ["Everything on the free board", "Price differences and market signals", "Context for recorded line movement"], action: "View account & availability", href: "/account" }, { plan: BILLING_PLANS.signals_spot_stats, caption: "Add a historical perspective.", items: ["Everything in Signals", "Planned historical situation research", "Sample sizes and methodology"], action: "View account & availability", href: "/account" }].map(({ plan, caption, items, action, href }, index) => <article className={index === 1 ? styles.featuredPlan : ""} key={plan.id}><div className={styles.planTop}><span>{plan.name}</span><small>{index ? "COMING SOON" : "START HERE"}</small></div><p className={styles.price}>${(plan.amount / 100).toFixed(index ? 2 : 0)}<span>{index ? "/ month" : " / always free"}</span></p><p>{caption}</p><ul>{items.map(item => <li key={item}><span aria-hidden="true">↗</span>{item}</li>)}</ul><Link href={href} className={index === 1 ? styles.primary : styles.planButton}>{action} <Arrow /></Link></article>)}
          </div>
          <p className={styles.pricingNote}>Prices in USD. Paid plans are previewed here; availability and purchase details appear in your account.</p>
        </section>
        <section className={styles.faq} aria-labelledby="faq-title"><div><p className={styles.eyebrow}>GOOD QUESTIONS</p><h2 id="faq-title">Before you<br />dive in.</h2></div><div>{[
          ...
          ["Can I subscribe today?", "Paid subscriptions are coming soon. You can explore the free market board now and check your account for plan availability."],
